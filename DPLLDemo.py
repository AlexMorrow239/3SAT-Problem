...
from Utilities import FormulaSimplifier, RandomFormulaGenerator
...
class DPLLVisualizer(DPLLSolver, BaseVisualizer):
    ...
    def _dpll(self, formula: Formula, assignments: dict) -> dict:
        ...
        self.color_print("\n" + "=" * 50, Fore.CYAN)
        self.color_print(
            f"Step {self.step_count} (Depth {current_depth}):", Fore.CYAN, Style.BRIGHT
        )
        ...
        if unit_clause:
            ...
            self.color_print(
                f"🔍 Found unit clause: {unit_clause}", Fore.GREEN, Style.BRIGHT
            )
        ...
        if pure_literal:
            ...
            self.color_print(
                f"🔍 Found pure literal: {pure_literal}", Fore.GREEN, Style.BRIGHT
            )
            self.color_print(
                f"Applying pure literal elimination: Setting {pure_literal}", Fore.GREEN
            )
            self.wait_for_user()
        ...
        if two_clause_assignment:
            ...
            self.color_print(
                f"Applying two-clause rule: Setting x{var} = {value}", Fore.GREEN
            )
            self.wait_for_user()
        ...
        # Variable selection
        self.color_print(
            "\nNo simplification rules apply. Choosing variable for branching...",
            Fore.BLUE,
        )
        var = self._choose_next_variable(formula)
        self.color_print(
            f"🎯 Choosing variable x{var} (most frequent)", Fore.BLUE, Style.BRIGHT
        )
        self.wait_for_user()
...
def main():
    ...
    for clause in clauses[1:]:
        ...
        if len(current_line + " ∧ " + clause) > (
            visualizer.term_width - 20
        ):  # Leave some margin
            formula_lines.append(current_line)
            current_line = clause
        else:
