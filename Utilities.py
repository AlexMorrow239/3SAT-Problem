...
from typing import (
    List, Dict, Set, Optional, Tuple, TypeVar, Final, 
    Iterator, Sequence, Mapping, Union, cast, overload
)
...
class Literal:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.variable, int):
            raise FormulaError(f"Variable must be an integer, got {type(self.variable)}")
        if self.variable < 1:
            raise FormulaError(f"Variable must be positive, got {self.variable}")
        if not isinstance(self.is_positive, bool):
            raise FormulaError(f"is_positive must be a boolean, got {type(self.is_positive)}")
    ...
    def polarity(self) -> LiteralPolarity:
        ...
        return LiteralPolarity.POSITIVE if self.is_positive else LiteralPolarity.NEGATIVE
    ...
    def __post_init__(self) -> None:
        ...
        for literal in self.literals:
            if not isinstance(literal, Literal):
                raise FormulaError(f"Each literal must be a Literal instance, got {type(literal)}")
...
class Formula:
    ...
    def __post_init__(self) -> None:
        ...
        for clause in self.clauses:
            if not isinstance(clause, Clause):
                raise FormulaError(f"Each clause must be a Clause instance, got {type(clause)}")
        
        if not isinstance(self.num_variables, int):
            raise FormulaError(f"Number of variables must be an integer, got {type(self.num_variables)}")
        if self.num_variables < 1:
            raise FormulaError(f"Number of variables must be positive, got {self.num_variables}")
        
        all_vars = self.get_all_variables()
        if all_vars and max(all_vars) > self.num_variables:
            raise FormulaError(f"Formula contains variable(s) beyond declared number: {max(all_vars)} > {self.num_variables}")
    ...
    def calculate_variable_frequencies(self) -> Counter:
        ...
        return Counter(lit.variable for clause in self.clauses for lit in clause.literals)
    ...
    def evaluate(self, assignment: Assignment) -> Optional[bool]:
        ...
        """
        results: List[Optional[bool]] = [clause.evaluate(assignment) for clause in self.clauses]
        if None in results:
            return None
...
class FormulaSimplifier:
    ...
    def simplify_formula(formula: Formula, assignment: Assignment) -> Formula:
        ...
        if not isinstance(assignment, dict):
            raise FormulaError(f"Assignment must be a dictionary, got {type(assignment)}")
        
        for var, val in assignment.items():
            if not isinstance(var, int):
                raise FormulaError(f"Assignment keys must be integers, got {type(var)}")
            if not isinstance(val, bool):
                raise FormulaError(f"Assignment values must be booleans, got {type(val)}")
            if var < 1:
                raise FormulaError(f"Variable numbers must be positive, got {var}")
            if var > formula.num_variables:
                raise FormulaError(f"Assignment contains variable {var} beyond formula's declared number {formula.num_variables}")
...
class RandomFormulaGenerator:
    ...
    def generate(self, num_variables: int, num_clauses: int) -> Formula:
        ...
        if not isinstance(num_variables, int) or num_variables < 3:
            raise FormulaError(f"Number of variables must be an integer ≥ 3, got {num_variables}")
        if not isinstance(num_clauses, int) or num_clauses < 1:
            raise FormulaError(f"Number of clauses must be a positive integer, got {num_clauses}")
        
        try:
            clauses: List[Clause] = []
            for _ in range(num_clauses):
                # Select 3 distinct variables
                vars_selected: List[int] = self.rng.sample(range(1, num_variables + 1), 3)
    ...
    def generate_phase_transition_formulas(
        ...
        if ratio_range[0] >= ratio_range[1]:
            raise FormulaError(f"Invalid ratio range: {ratio_range[0]} >= {ratio_range[1]}")
        if not isinstance(num_ratios, int) or num_ratios < 1:
            raise FormulaError(f"num_ratios must be a positive integer, got {num_ratios}")
        if not isinstance(formulas_per_ratio, int) or formulas_per_ratio < 1:
            raise FormulaError(f"formulas_per_ratio must be a positive integer, got {formulas_per_ratio}")
