...
class ComprehensiveSATAnalyzer:
    ...
    def __init__(self):
        ...
        self.solvers = {
            ...
            "random": RandomSATSolver(
                max_flips=100, max_tries=10
                ...
                trial_results.update(
                    {
                        ...
                        "unit_propagations": solver.stats.stats[
                            "unit_propagations"
                        ].value,
                        "pure_literals": solver.stats.stats["pure_literals"].value,
                        "decision_depths_avg": np.mean(
                            solver.stats.stats["decision_depths"].value
                        )
                        if solver.stats.stats["decision_depths"].value
                        else 0,
                    }
                )
                ...
                trial_results.update(
                    {
                        ...
                        "successful_flips": solver.stats.stats[
                            "successful_flips"
                        ].value,
                ...
                trial_results.update(
                    {
                        ...
                        "assignments_tested": solver.stats.stats[
                            "assignments_tested"
                        ].value,
                        "partial_validations": solver.stats.stats[
                            "partial_validations"
                        ].value,
                    }
                )
    ...
    def _plot_phase_transition(self, df: pd.DataFrame) -> None:
        ...
        plt.axvspan(
            min(df["ratio"]), 4.26, alpha=0.1, color="green", label="SAT Region"
        )
        plt.axvspan(
            4.26, max(df["ratio"]), alpha=0.1, color="red", label="UNSAT Region"
        )
    ...
    def _plot_time_complexity(self, df: pd.DataFrame) -> None:
        ...
        small_n = [
            n for n in all_n if n <= 15
        ]  # n values where all solvers were tested
        ratios = sorted(df["ratio"].unique())
        mid_ratio = ratios[len(ratios) // 2]

        colors = {"dpll": "#2E86AB", "random": "#A23B72", "exhaustive": "#F18F01"}
        ...
        plt.title(
            f"Solver Time Complexity Analysis\n(at α ≈ {mid_ratio:.2f})",
            fontsize=14,
            pad=20,
        )
    ...
    def _plot_heatmaps(self, df: pd.DataFrame) -> None:
        ...
        gs = plt.GridSpec(
            2, 2, height_ratios=[4, 1], width_ratios=[5, 5], hspace=0.3, wspace=0.3
        )
        ...
        for idx, (metric, title) in enumerate(metrics.items()):
            ...
            pivot = df.pivot_table(
                values=metric, index="n", columns="ratio", aggfunc="mean"
            )
        ...
        for idx, (box, text) in enumerate(
            zip(insight_boxes, [sat_insights, depth_insights])
        ):
            box.text(
                0.05,
                0.5,
...
def run_comprehensive_analysis():
        ...
        for ratio in ratios:
            ...
            for _ in range(trials_per_config):
                analyzer.compare_solvers(
                    n, ratio, results, solvers_to_run=["dpll", "random", "exhaustive"]
                )
    ...
    for n in large_n:
        ...
        for ratio in ratios:
            print(f"  m/n = {ratio:.2f}", end="\r")
            for _ in range(trials_per_config):
                analyzer.compare_solvers(n, ratio, results, solvers_to_run=["dpll"])
